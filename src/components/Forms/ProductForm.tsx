...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const {
    ...
    formState: { errors },
    reset,
    // watch // No longer needed for conditional Qty/Case (L)
  ...
  useEffect(() => {
    if (product) {
      reset({
        ...
        qtyPerCaseL: product.qtyPerCaseL,
      });
    } else {
  ...
  const onFormSubmit = (data: ProductFormData) => {
    const submittedData = { ...data } as Partial<ProductFormData & { qtyPerCaseL: number | undefined | string }>;
    ...
    onSubmit(submittedData as Partial<Product>);
  };
  ...
  const formFields = [
    { name: 'name' as const, label: 'Product Name', type: 'text', placeholder: 'Enter product name', required: 'Product name is required', colSpan: 'sm:col-span-2' },
    ...
    { name: 'apiSpec' as const, label: 'API Specification', type: 'text', placeholder: 'e.g., API-XYZ123', required: 'API Spec is required', colSpan: 'sm:col-span-1' },
    { name: 'packSize' as const, label: 'Pack Size', type: 'text', placeholder: 'e.g., 500ml, 1kg', required: 'Pack size is required', colSpan: 'sm:col-span-1' },
    // { name: 'price' as const, label: 'Price ($)', type: 'number', placeholder: '0.00', required: 'Price is required', min: { value: 0, message: 'Price must be positive' }, colSpan: 'sm:col-span-1' }, // Removed Price
    { name: 'uom' as const, label: 'Unit of Measure (UOM)', type: 'text', placeholder: 'e.g., ml, kg, pcs', required: 'UOM is required', colSpan: 'sm:col-span-1' },
    ...
    { name: 'qtyPerCaseL' as const, label: 'Qty per Case (L)', type: 'number', placeholder: 'e.g., 12.5 (Optional)', required: false, min: { value: 0, message: 'Must be positive' }, colSpan: 'sm:col-span-1' },
  ];
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
        ...
        {/* Removed conditional rendering for QtyPerCaseL as it's now part of formFields */}
      </div>
      ...
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3, delay: (formFields.length + 1) * 0.05 }} // Adjusted delay index
        className="flex flex-col sm:flex-row gap-3 pt-2"
      >
        <button
          type="submit"
          disabled={isSubmitting}
          className="flex-1 bg-blue-600 text-white px-4 py-2.5 rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation text-sm"
        >
